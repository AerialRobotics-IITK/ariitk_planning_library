...
class Visualizer {
    ...
    private:
        visualization_msgs::Marker createMarker(const std::string& ns, const ColorType& color, const double& scale,
                        const uint& type, const std::string& frame_id = "world", const uint& id = 0, const uint& action = visualization_msgs::Marker::ADD);
